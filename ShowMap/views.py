...
#  * @desc [description]
def uclid_process(data_merge,jeju_pop,spot_data):
    # 결과 데이터프레임 초기화
    result_data = pd.DataFrame(columns=['장소명', '자전거도로', '지하철역', '공원', '관광지','대학','총이동'])
...
def jeju_analysis(request):
    ...
    jeju_pop = list(JejuPop.objects.all())
    ...
    result_data = uclid_process(jeju_facility_df,jeju_pop,spot_data)
    ...
    # # pkl 파일 경로
    pkl_file_path = "C://Users//yjs60//Documents//GitHub//team1_main//ShowMap//static//model//knn_model_6_stdX.h5"
    import pickle
    # pkl 파일 로드
    model=0
    ...
    # 모델 로드
    #import joblib
    #model = joblib.load('static/model/MLP.h5') 
